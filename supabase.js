...
function ensureConnectionUI(){
  ...
  const panel=document.createElement('div'); panel.id='lfConnectionPanel'; panel.innerHTML=`
    <div class="lf-connection-dot"></div><div class="lf-connection-copy">
      <strong id="lfSyncStatus">Local-first mode</strong><span id="lfSyncDetail" style="display:none">Changes are saved on this device.</span>
...
async function handleSession(session){
  ...
  try{
    ...
    /* Some deployed schemas do not return the nested roles(name) relation.
       Resolve role_id directly so a real super admin is never downgraded to
       a blank/read-only role in the browser. */
    if(!profile.roles?.name && profile.role_id){
      try{
        const roleRow=await labourForceSupabase.from('roles').select('name').eq('id',profile.role_id).maybeSingle();
        if(roleRow.data?.name)profile.roles={...(profile.roles||{}),name:roleRow.data.name};
      }catch(roleError){console.warn('[Labour Force] role lookup failed:',roleError.message||roleError);}
    }
...
async function lfSaveAttendanceDate(date){if(!date||!attendance[date])return;const day=attendance[date];if(!day||!day.records)return;for(const[localWorkerId,r]of Object.entries(day.records||{})){if(!r||r.status==='pending')continue;const w=workers.find(wk=>Number(wk.id)===Number(localWorkerId));if(!w)continue;const wRemote=workerRemote(localWorkerId);if(!wRemote)continue;const status=r.status==="present"||r.status==="worked"||r.status==="approved"?"present":r.status==="pending"?"pending":"absent";const dep=typeof deployments!=="undefined"?deployments.find(d=>Number(d.workerId)===Number(localWorkerId)&&d.status==="Active"):null;const batchName=r.batchName||'Default';const full={attendance_date:date,worker_id:wRemote,deployment_id:dep?rid("deployment",dep.id):null,client_id:dep?clientRemote(dep.clientId):null,department_id:deptRemote(w.department),status,overtime_hours:Number(r.overtime||0),regular_hours:Number(r.hours||0),batch_name:batchName,remarks:r.remarks||r.notes||null,verification_status:r.verification_status==="verified"?"verified":"unverified",verified_by:r.verified_by_id||null,verified_at:r.verified_at||null,created_by:profileId(),updated_by:profileId()};const drops=[[],["verification_status","verified_by","verified_at"],["remarks","verification_status","verified_by","verified_at"],["batch_name","remarks","verification_status","verified_by","verified_at"],["regular_hours","batch_name","remarks","verification_status","verified_by","verified_at"]];let saved=false;for(const drop of drops){const row={...full};drop.forEach(k=>delete row[k]);const conflict=drop.includes("batch_name")?"worker_id,attendance_date":"worker_id,attendance_date,batch_name";try{const res=await labourForceSupabase.from("attendance").upsert(row,{onConflict:conflict}).select("id").single();if(res.error){if(res.error.code==="42703"||res.error.code==="42P10"||/column|conflict|constraint/i.test(res.error.message||""))continue;throw res.error;}if(res.data?.id){const map=lfMap();map.attendance=map.attendance||{};map.attendance[date+":"+localWorkerId+":"+batchName]=res.data.id;saveLfMap(map);}saved=true;break;}catch(e){if(e&& (e.code==="42703"||e.code==="42P10"||/column|conflict|constraint/i.test(e.message||"")))continue;console.warn("[LF] lfSaveAttendanceDate:",e.message||e);break;}}if(!saved)console.warn("[LF] lfSaveAttendanceDate: no compatible column set for date",date);}}
async function lfSaveAttendanceApproval(date,mode){if(!date)return;const day=attendance[date]||{};if(mode==="submit"){const payload={attendance_date:date,status:"submitted",submitted_by:profileId(),submitted_at:day.submittedAt||new Date().toISOString()};try{await labourForceSupabase.from("attendance_approvals").upsert(payload,{onConflict:"attendance_date"}).select("id").single();}catch(e){console.warn("[LF] lfSaveAttendanceApproval submit:",e.message);}}else if(mode==="approve"){const payload={attendance_date:date,status:"approved",approved_by:profileId(),approved_at:day.approvedAt||new Date().toISOString(),submitted_at:day.submittedAt||null};try{await labourForceSupabase.from("attendance_approvals").upsert(payload,{onConflict:"attendance_date"}).select("id").single();}catch(e){console.warn("[LF] lfSaveAttendanceApproval approve:",e.message);}}}
  ...
  lfHydrateInFlight=(async()=>{
    ...
    try{
      ...
      const [rc,rd,rwPublic,rr]=await Promise.all([
        ...
        safeTableRows('workers_public','id,employee_no,full_name,phone,national_id,id_number,department_id,classification,designation,active'),
      ...
      const rw = rwPublic.length ? rwPublic.map(pub=>{
        ...
        return {
          id:pub.id, employee_no:rate.employee_no||pub.employee_no||pub.staff_no, staff_no:pub.staff_no||pub.employee_no,
          full_name:rate.full_name||pub.full_name||pub.name, name:pub.name||pub.full_name, phone:rate.phone||pub.phone||null,
          national_id:rate.national_id||pub.national_id||pub.id_number||null, id_number:pub.id_number||rate.id_number||pub.national_id||null,
          kra_pin:rate.kra_pin||null, nssf_number:rate.nssf_number||null, shif_number:rate.shif_number||null,
          account_number:rate.account_number||null, department_id:rate.department_id||pub.department_id||null, department:pub.department||null,
          classification:rate.classification||pub.classification||null, designation:rate.designation||pub.designation||null,
          daily_rate:rate.daily_rate, overtime_rate:rate.overtime_rate, join_date:rate.join_date||pub.join_date||null,
  ...
  const wrapped=function(){
    ...
    if(typeof LF_PHASE2!='undefined'&&LF_PHASE2===true){
      if(/submitAttendance|approveAttendance|submitSupervisorAttendance/.test(stack)){
        const dEl=document.getElementById('approvalDate')||document.getElementById('attendanceDate')||document.getElementById('supervisorDate')||document.getElementById('supDate')||document.getElementById('jtsDate');
      ...
      if(ATT_MUTATORS.test(stack)){
        const dEl=document.getElementById('attendanceDate')||document.getElementById('supervisorDate')||document.getElementById('supDate')||document.getElementById('jtsDate');
        const date=dEl?.value||new Date().toISOString().slice(0,10);
        lfSaveAttendanceDate(date).catch(e=>console.warn('[LF] lfSaveAttendanceDate:',e.message));
        return;
      }
